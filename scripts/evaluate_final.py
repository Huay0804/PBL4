import argparse
import json
import os
import sys
from pathlib import Path

import gc
import keras
import numpy as np

PROJECT_ROOT = Path(__file__).resolve().parents[1]
SCRIPTS_DIR = PROJECT_ROOT / "scripts"
if str(SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_DIR))

from train import (
    DEFAULT_INPUT_HEIGHT,
    DEFAULT_INPUT_WIDTH,
    compute_confusion_matrix,
    infer_num_classes,
    list_pairs,
    load_class_map,
    make_dataset,
    report_group_metrics,
    report_per_class_metrics,
)
from helper_functions import (
    MeanIoUMetric,
    bce_dice_loss,
    ce_dice_loss,
    ce_dice_boundary_loss,
    dice_coef,
    dice_coef_loss,
    iou_score,
    mean_iou,
    multiclass_dice_loss,
)
from project_presets import DEFAULT_SEGMENTATION_MODEL, get_segmentation_preset
from protocol_utils import (
    resolve_segmentation_checkpoint,
    summarize_mask_paths,
    validate_bb_map_files,
    write_json,
)


SPLITS_DIR = Path(os.environ.get("PBL4_SPLITS_DIR", "data/splits"))
CLASS_MAP_PATH = Path(os.environ.get("PBL4_CLASS_MAP_PATH", "data/splits/class_map.txt"))
RUNS_DIR = Path("runs")
YOLOX_BB_MAPS_ROOT = Path(os.environ.get("PBL4_YOLOX_BB_MAPS_DIR", "data/bb_maps/yolox"))
MASK_RCNN_BB_MAPS_ROOT = Path(os.environ.get("PBL4_MASK_RCNN_BB_MAPS_DIR", "data/bb_maps/mask_rcnn"))
INCLUDE_BACKGROUND = False


def _is_multi_output_model(model):
    return len(getattr(model, "outputs", [])) > 1


def _build_inference_model(model):
    if not _is_multi_output_model(model):
        return model, None, None
    output_names = list(model.output_names)
    selected_name = "deep_supervision_average"
    inference_output = keras.layers.Average(name=selected_name)(model.outputs)
    inference_model = keras.Model(model.inputs, inference_output, name=f"{model.name}_inference")
    return inference_model, output_names, selected_name


def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate a saved segmentation model on the fixed test set using project presets."
    )
    parser.add_argument(
        "--model",
        default=DEFAULT_SEGMENTATION_MODEL,
        choices=[
            "mod_nestnet",
            "icpr_munet",
            "transunet",
        ],
    )
    parser.add_argument("--batch-size", type=int, default=None, help=argparse.SUPPRESS)
    parser.add_argument(
        "--loss",
        choices=["ce_dice", "ce_dice_boundary", "bce_dice", "dice"],
        default=None,
        help=argparse.SUPPRESS,
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
        help=(
            "Override model run root/directory. Supports both legacy layout "
            "(best.keras/last.keras directly inside) and timestamped layout "
            "(with latest pointer/sub-runs)."
        ),
    )
    parser.add_argument(
        "--cv-fold",
        type=int,
        choices=range(4),
        help="Evaluate a CV fold run (runs/cv/fold_<n>/<model>/).",
    )
    parser.add_argument(
        "--checkpoint-policy",
        choices=["best", "last"],
        default="best",
        help="Checkpoint selection policy when --checkpoint-path is not provided.",
    )
    parser.add_argument(
        "--checkpoint-path",
        type=Path,
        help="Explicit checkpoint path to evaluate.",
    )
    parser.add_argument(
        "--bb-source",
        choices=("yolox", "mask_rcnn", "legacy_splits"),
        default=os.environ.get("PBL4_BB_SOURCE"),
        help=(
            "BB-map source used for BB-gated models. "
            "'yolox' -> data/bb_maps/yolox/test/bb_maps, "
            "'mask_rcnn' -> data/bb_maps/mask_rcnn/test/bb_maps, "
            "'legacy_splits' -> data/splits/test/bb_maps."
        ),
    )
    parser.add_argument(
        "--bb-maps-dir",
        type=Path,
        default=None,
        help="Optional explicit BB-maps directory (contains .npz files). Overrides --bb-source.",
    )
    args = parser.parse_args()
    preset = get_segmentation_preset(args.model)
    if args.batch_size is None:
        args.batch_size = preset["batch_size"]
    if args.loss is None:
        args.loss = preset["loss"]
    if args.bb_source is None:
        args.bb_source = preset.get("bb_source", "yolox")
    return args


def _resolve_test_bb_maps_dir(args) -> Path:
    if args.bb_maps_dir is not None:
        return args.bb_maps_dir
    if args.bb_source == "legacy_splits":
        return SPLITS_DIR / "test" / "bb_maps"
    if args.bb_source == "mask_rcnn":
        return MASK_RCNN_BB_MAPS_ROOT / "test" / "bb_maps"
    return YOLOX_BB_MAPS_ROOT / "test" / "bb_maps"


def _summarize_overall(iou, dice, support, include_background):
    start = 0 if include_background else 1
    idxs = list(range(start, len(iou)))
    if not idxs:
        return {}
    pixels = support[idxs]
    pixel_sum = int(pixels.sum())

    macro_iou = float(np.mean(iou[idxs]))
    macro_dice = float(np.mean(dice[idxs]))
    if pixel_sum > 0:
        weighted_iou = float(np.sum(iou[idxs] * pixels) / pixel_sum)
        weighted_dice = float(np.sum(dice[idxs] * pixels) / pixel_sum)
    else:
        weighted_iou = 0.0
        weighted_dice = 0.0

    worst_idx = min(idxs, key=lambda idx: iou[idx])
    return {
        "macro_iou": macro_iou,
        "macro_dice": macro_dice,
        "weighted_iou": weighted_iou,
        "weighted_dice": weighted_dice,
        "worst_class_id": int(worst_idx),
        "worst_iou": float(iou[worst_idx]),
        "worst_dice": float(dice[worst_idx]),
        "pixels": pixel_sum,
        "include_background": include_background,
    }


def _select_loss(num_classes, loss_name):
    if num_classes == 1:
        if loss_name == "dice":
            return dice_coef_loss
        if loss_name == "bce_dice":
            return bce_dice_loss
        if loss_name == "ce_dice_boundary":
            return ce_dice_boundary_loss
        return ce_dice_loss
    if loss_name == "dice":
        return multiclass_dice_loss
    if loss_name == "ce_dice_boundary":
        return ce_dice_boundary_loss
    return ce_dice_loss


def _select_metrics(num_classes):
    if num_classes == 1:
        return [dice_coef, mean_iou, iou_score]
    return [
        keras.metrics.SparseCategoricalAccuracy(name="acc"),
        MeanIoUMetric(num_classes=num_classes, name="mean_iou"),
    ]


def main():
    args = parse_args()
    keras.backend.clear_session()
    gc.collect()
    run_name = args.model
    if args.run_dir:
        run_dir = args.run_dir
    elif args.cv_fold is not None:
        run_dir = RUNS_DIR / "cv" / f"fold_{args.cv_fold}" / run_name
    else:
        run_dir = RUNS_DIR / run_name
    model_path, checkpoint_info = resolve_segmentation_checkpoint(
        run_dir,
        checkpoint_policy=args.checkpoint_policy,
        checkpoint_path=args.checkpoint_path,
    )

    loaded_model = keras.models.load_model(model_path, compile=False)
    model, deep_supervision_outputs, deep_supervision_selected_output = _build_inference_model(
        loaded_model
    )
    if deep_supervision_selected_output is not None:
        print(
            "Deep supervision checkpoint detected; "
            f"evaluation output={deep_supervision_selected_output}"
        )

    num_classes = infer_num_classes(CLASS_MAP_PATH) or 1
    output_channels = model.output_shape[-1]
    if output_channels not in (None, 1) and output_channels != num_classes:
        print(
            f"warning: model output classes ({output_channels}) "
            f"do not match class_map ({num_classes})"
        )

    size = (DEFAULT_INPUT_HEIGHT, DEFAULT_INPUT_WIDTH)
    preprocess_fn = None
    scale_to_255 = False

    bb_maps_dir = None
    if len(model.inputs) > 1:
        bb_maps_dir = _resolve_test_bb_maps_dir(args)
        if not bb_maps_dir.exists():
            raise SystemExit(f"BB maps directory not found: {bb_maps_dir}")

    bb_channels = None
    if len(model.inputs) > 1:
        inferred = model.inputs[1].shape[-1]
        bb_channels = inferred if inferred is not None else num_classes

    test_pairs = list_pairs(
        SPLITS_DIR / "test" / "img",
        SPLITS_DIR / "test" / "masks_semantic",
        bb_maps_dir=bb_maps_dir,
    )
    if not test_pairs:
        raise SystemExit("No test image/mask pairs found.")
    test_mask_stats = summarize_mask_paths(
        [pair[1] for pair in test_pairs], num_classes, subset_name="test"
    )
    test_bb_stats = None
    if bb_maps_dir is not None:
        test_bb_stats = validate_bb_map_files(
            [pair[2] for pair in test_pairs],
            expected_height=DEFAULT_INPUT_HEIGHT,
            expected_width=DEFAULT_INPUT_WIDTH,
            expected_channels=bb_channels if bb_channels is not None else num_classes,
            subset_name="test",
        )

    test_ds = make_dataset(
        test_pairs,
        size=size,
        num_classes=num_classes,
        batch_size=args.batch_size,
        shuffle=False,
        augment=False,
        preprocess_fn=preprocess_fn,
        scale_to_255=scale_to_255,
        bb_channels=bb_channels if len(model.inputs) > 1 else None,
    )

    loss = _select_loss(num_classes, args.loss)
    metrics = _select_metrics(num_classes)
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=1e-4),
        loss=loss,
        metrics=metrics,
        jit_compile=False,
    )

    report = model.evaluate(test_ds, verbose=1, return_dict=True)

    out_dir = model_path.parent
    out_dir.mkdir(parents=True, exist_ok=True)
    write_json(out_dir / "test_metrics.json", report)
    print("Test metrics:", report)

    class_names = load_class_map(CLASS_MAP_PATH)
    class_names.setdefault(0, "background")

    # Free dataset memory before computing confusion matrix, then re-create
    # a small-batch dataset to reduce peak memory usage.
    del test_ds
    gc.collect()

    cm_ds = make_dataset(
        test_pairs,
        size=size,
        num_classes=num_classes,
        batch_size=1,
        shuffle=False,
        augment=False,
        preprocess_fn=preprocess_fn,
        scale_to_255=scale_to_255,
        bb_channels=bb_channels if len(model.inputs) > 1 else None,
    )
    confusion = compute_confusion_matrix(cm_ds, model, num_classes)
    iou, dice, support = report_per_class_metrics("test", confusion, class_names, out_dir)
    report_group_metrics("test", iou, dice, support, out_dir, num_classes)

    summary = _summarize_overall(iou, dice, support, INCLUDE_BACKGROUND)
    write_json(out_dir / "test_summary.json", summary)
    write_json(
        out_dir / "test_evaluation_metadata.json",
        {
            "run_dir": str(run_dir),
            "model_path": str(model_path),
            "checkpoint": {
                **checkpoint_info,
                "path": str(model_path),
            },
            "model": args.model,
            "encoder": "icpr",
            "deep_supervision": bool(deep_supervision_outputs),
            "deep_supervision_outputs": deep_supervision_outputs,
            "deep_supervision_eval_output": (
                deep_supervision_selected_output if deep_supervision_outputs else None
            ),
            "bb_source": args.bb_source if len(model.inputs) > 1 else None,
            "bb_maps_dir": str(bb_maps_dir) if bb_maps_dir is not None else None,
            "num_classes": int(num_classes),
            "test": {
                "samples": len(test_pairs),
                **test_mask_stats,
                **({"bb_maps": test_bb_stats} if test_bb_stats is not None else {}),
            },
        },
    )
    print(f"Wrote test summary to {out_dir / 'test_summary.json'}")

    keras.backend.clear_session()
    gc.collect()


if __name__ == "__main__":
    main()
